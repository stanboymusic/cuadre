...
const PRICE_TIERS = [
  ...
  // "Precio personalizado" no se calcula desde el PB: se escribe a mano por
  // producto y se guarda en el campo (reutilizado) products.price.
  { key: 'custom', label: 'Precio personalizado', margin: null },
];
function tierPrice(product, tierKey) {
  const tier = PRICE_TIERS.find(t => t.key === tierKey) || PRICE_TIERS[0];
  if (tier.key === 'custom') return Number(product && product.price) || 0;
